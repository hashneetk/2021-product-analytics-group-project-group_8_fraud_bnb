...
def get_info(url, waiting_time=3, max_review=10):
    ...
    result : dict
        A dictionary with the following info:
            ...
            - beds (list of str): description of the
                                  number of bed in each room.
    ...
    result = {
        ...
        'beds': [room.text
                 for room in driver
                     .find_elements_by_css_selector('div._9342og')],
    ...
    result['description'] = driver\
        .find_element_by_css_selector('div._15pb00k')\
        .text

    review_url = driver.find_element_by_css_selector('div._19qg1ru a')\
        .get_attribute('href')
    driver.get(review_url)
    time.sleep(waiting_time)

    rating, review_ct = driver\
        .find_elements_by_css_selector('h2._14i3z6h')[-1].text.split(' (')

    result['rating'] = float(rating)
    result['review_ct'] = int(review_ct.split(' ')[0])

    reviews_elements = driver\
        .find_elements_by_css_selector('div._1gjypya')
